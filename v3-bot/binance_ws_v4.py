"""
Binance Price Feed — WebSocket with REST fallback.

Tries WebSocket first. If blocked (451/geo-restricted), automatically
falls back to REST polling every 1 second. The public interface
(get_price, get_tick_history, get_realized_vol) works identically.
"""
...
def _run_rest_poller():
    """Poll Binance REST every 1s as fallback when WS is unavailable."""
    import httpx
    client = httpx.Client(timeout=3.0)
    logger.info("[REST] Binance REST poller started (1s interval)")
    while True:
